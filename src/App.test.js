...
const restartButton = getByTestId(testIds.restartButton); expect(restartButton).toHaveTextContent("Restart"); 
...
test('Switching between slides works as expected', () => { const slides = makeSlides(5); 
const { getByTestId } = renderApp(slides); 
const restartButton = getByTestId(testIds.restartButton); const prevButton = getByTestId(testIds.prevButton); const nextButton = getByTestId(testIds.nextButton); const titleElem = getByTestId(testIds.title); 
const textElem = getByTestId(testIds.text); 
const clicks = [ 
...
{ 
expect(restartButton).toBeDisabled(); expect(prevButton).toBeDisabled(); } 
if (idx+1 < slides.length) { 
expect(nextButton).toBeEnabled(); } else { 
expect(nextButton).toBeDisabled(); } 
const { title, text } = slides[idx]; expect(titleElem).toHaveTextContent(title); expect(textElem).toHaveTextContent(text); } });
